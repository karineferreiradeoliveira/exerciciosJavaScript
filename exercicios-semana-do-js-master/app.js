...
const randomNumbers = [10, 30, 15, 25, 50, 40, 5]
    ...
    const imPar = randomNumbers.filter(item => (item%2 == 1))
    console.log(imPar)
...
const lan = tarantinoMovies.filter( item => item.release <= 2000)
console.log(lan)
...
const film = tvShows.map(tvShow => {
      if (tvShows.name = true) {
...
const jogos = cart.reduce((accumulator,{name}) => 
  `${accumulator}- ${name}\n` , '')
